...
app.get('/team_stats', function(req, res) {
  ...
  var game_wins = "SELECT COUNT(*) AS wins FROM football_games WHERE home_score > visitor_score;";
  var game_losses = "SELECT COUNT(*) AS losses FROM football_games WHERE home_score < visitor_score;";
  db.task('get-everything', task => {
    return task.batch([
      task.any(all_games),
      task.any(game_wins),
      task.any(game_losses)
    ]);
  })
  .then(info => {
...
app.get('/player_info/select_player', function(req, res) {
  ...
  var grab_all_players = 'select id, name from football_players;';
	var grab_player = "select * from football_players where id = " + player_id + ";";
  var grab_number_games = "select count(*) from football_games where " + player_id + " = any(players);";
	db.task('get-everything', task => {
        return task.batch([
            task.any(grab_all_players),
            task.any(grab_player),
            task.any(grab_number_games)
        ]);
    })
    .then(info => {
    	res.render('pages/player_info',{
				title: "Player Info",
				players: info[0],
        selected_player: info[1][0],
        number_games: info[2][0]
			})
    })
    .catch(error => {
        // display error message in case an error
